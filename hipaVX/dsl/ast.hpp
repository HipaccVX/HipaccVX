...
/**
 * @brief The VectChannelType enum describes the channels for a vectorlike
 * datatype
 */
enum class VectChannelType { CHANNEL0, CHANNEL1, CHANNEL2, CHANNEL3 };
...
class Node {
 public:
  int id; /**< This gets set when constructing a new Node Instance. This is read
             only */
  Node() {
 ...
 *
 * It is a placeholder node, which gets linked in the Abstraction Layers to the
 * actual values and instances
 */
class VariableAccessor : public Node {
 public:
  int num;
  VariableAccessor() { type = NodeType::VariableAccessor; }
  VariableAccessor(int number) : num(number) {
...
class PixelAccessor : public Node {
 public:
  int num;
  PixelAccessor() { type = NodeType::PixelAccessor; }
  PixelAccessor(int number) : num(number) { type = NodeType::PixelAccessor; }
};
 ...
 * etc..\n The left argument has to be stored in subnodes[0] and the right
 * argument in subnodes[1]. The actual operation is deduced from the Nodes type
 */
...
class ShiftLeft : public SimpleBinaryNode {
 public:
  ...
  ShiftLeft(std::shared_ptr<ast4vx::Node> n1,
            std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::ShiftLeft;
    subnodes = {n1, n2};
  }
...
class ShiftRight : public SimpleBinaryNode {
 public:
  ...
  ShiftRight(std::shared_ptr<ast4vx::Node> n1,
             std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::ShiftRight;
    subnodes = {n1, n2};
  }
...
class BitwiseAnd : public SimpleBinaryNode {
 public:
  ...
  BitwiseAnd(std::shared_ptr<ast4vx::Node> n1,
             std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::BitwiseAnd;
    subnodes = {n1, n2};
  }
...
class BitwiseOr : public SimpleBinaryNode {
 public:
  ...
  BitwiseOr(std::shared_ptr<ast4vx::Node> n1,
            std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::BitwiseOr;
    subnodes = {n1, n2};
  }
...
class BitwiseXor : public SimpleBinaryNode {
 public:
  ...
  BitwiseXor(std::shared_ptr<ast4vx::Node> n1,
             std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::BitwiseXor;
    subnodes = {n1, n2};
  }
...
class LessEquals : public SimpleBinaryNode {
 public:
  ...
  LessEquals(std::shared_ptr<ast4vx::Node> n1,
             std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::LessEquals;
    subnodes = {n1, n2};
  }
...
class GreaterEquals : public SimpleBinaryNode {
 public:
  ...
  GreaterEquals(std::shared_ptr<ast4vx::Node> n1,
                std::shared_ptr<ast4vx::Node> n2) {
    type = NodeType::GreaterEquals;
    subnodes = {n1, n2};
  }
 ...
 *
 * The conversion argument has to be stored in subnods[0], as all other
 * SimpleUnaryFunctioNode Its destination type has to be written into the member
 * variable Conversion::to
 */
class Conversion : public SimpleUnaryFunctionNode {
 public:
  ...
  Conversion(std::shared_ptr<Node> n1, Datatype d) : to(d) {
    type = NodeType::Conversion;
    subnodes = {n1};
  }
  Datatype to;
 ...
 *
 * The entries are in the Node::subnodes member variable and the datatype is set
 * by Vect4::to_dtype
 */
class Vect4 : public Node {
 public:
  ...
  Vect4(std::shared_ptr<ast4vx::Node> n1, std::shared_ptr<ast4vx::Node> n2,
 ...
 *
 * The source Vect4 is stored in subnodes[0] and the channel is in
 * Extract4::channel
 */
...
class Constant : public Node {
 public:
  Constant() { type = NodeType::Constant; }
  Constant(T value) {
 ...
 *
 * The Variables name is stored in Variable::name and its Datatype is in
 * Variable::datatype
 */
 ...
 *
 * The left hand values has to be stored in Node::subnodes[0] and the right hand
 * value expression in Node::subnodes[1]
 ...
 * actual values and instances
 */
class WindowAccessor : public Node,
                       public std::enable_shared_from_this<WindowAccessor> {
 public:
  /**
   * @brief Abstract accessor class for a position in a window
   *
   * It is a placeholder node, which gets linked in the Abstraction Layers to
   * the actual values and instances
   */
  class Position : public Node {
   ...
   private:
    Position(std::shared_ptr<WindowAccessor> parent) : parent(parent) {
      type = NodeType::WindowAccessorPosition;
    }
    Position(unsigned int x, unsigned int y,
             std::shared_ptr<WindowAccessor> parent)
  ...
  int num;
  WindowAccessor() { type = NodeType::WindowAccessor; }
  WindowAccessor(int number) : num(number) { type = NodeType::WindowAccessor; }

  /**
   * @brief pixel_at Creates a placeholder node at x,y if it does not exist
   * already and returns it
   * @param x X Coordinate (0-indexed)
   * @param y Y Coordinate (0-indexed)
   * @return A placeholder node for the corresponding pixel at the current
   * coordinate in the window
   */
  std::shared_ptr<Position> pixel_at(unsigned int x, unsigned int y) {
    auto it = std::find_if(
        accessors.begin(), accessors.end(),
        [x, y](std::shared_ptr<Position> m) { return m->x == x && m->y == y; });
    if (it != accessors.end()) return *it;
 ...
 public:
  ...
  std::shared_ptr<VariableAccessor> v_in(unsigned int index) {
    if (index >= in_variable_mappings.size())
      throw std::runtime_error(
          "std::shared_ptr<VariableAccessor> Statements::v_in(unsigned int "
          "index): Index out of bounds");
  ...
  void set_data_inout_count(unsigned int out, unsigned int in) {
    ...
    unsigned int i = 0;
    for (; i < out; i++) out_pixel_mappings[i].reset(new PixelAccessor(i));
...
/**
 * @brief Packs AST Nodes together into many statements, which work on a local
 * to pixel basis
 *
 * It provides different accessors for the AST and saves statements, which can
 * be any AST Node.\n A Statement therefore should be a self contained AST
 * Operation. This can be imagined as a line in C or C++. Its difference to
 ...
 public:
  ...
  std::shared_ptr<WindowAccessor> w_in(unsigned int index) {
    if (index >= windows.size())
      throw std::runtime_error(
          "LocalToPixel::w_in(unsigned int index): Index out of bounds");
...
class MaskAccessor : public Node {
 public:
  int num;
  MaskAccessor() { type = NodeType::MaskAccessor; }
  MaskAccessor(int number) : num(number) { type = NodeType::MaskAccessor; }
};

/**
 * @brief Packs AST Nodes together into many statements, which work on a pixel
 * to pixel basis with mask values
 *
 * It provides different accessors for the AST and saves statements, which can
 * be any AST Node.\n A Statement therefore should be a self contained AST
 * Operation. This can be imagined as a line in C or C++. Its difference to
...
class MaskPixelToPixel : public PixelToPixel {
 public:
  ...
  /**
   * @brief m_in Accessing a (m)ask (in)put mapping, it corresponds to the
   * correct current mask value of the mask
  ...
  std::shared_ptr<MaskAccessor> m_in(unsigned int index) {
    if (index >= masks.size())
      throw std::runtime_error(
          "MaskPixelToPixel::m_in(unsigned int index): Index out of bounds");
    return masks[index];
  }
};

/**
 * @brief Packs AST Nodes together into many statements, which abstracts a
 * reduction operation
 *
...
class WindowDescriptor : public Node {
 public:
  ...
  WindowDescriptor(unsigned int x, unsigned int y,
    ...
    if (y <= 0 || x <= 0)
      throw std::runtime_error(
          "WindowDescriptor::WindowDescriptor domain size must be > 0");
...
class WindowOperation : public Node,
 ...
 public:
  ...
  /**
   * @brief Computes a PixelToPixel Statement at a specific given coordinate in
   * the domain
   * @param x X Coordinate in the domain (0-indexed)
   * @param y Y Coordinate in the domain (0-indexed)
   * @param s The PixelToPixel Statement that should be computed at the given
   * coordinates
   ...
   * @throws std::runtime_error will be thrown when the domain size is not known
   * yet. This happens when the size is not set explicitly and
   * WindowOperation::set_window_inputs() haven't been called yet with at least
   * one WindowDescriptor
   * @throws std::runtime_error will be thrown when the coordinates are out of
   * bounds
   ...
   * @param s The PixelToPixel Statement that should be computed for the whole
   * domain
   * @throws std::runtime_error will be thrown when this WindowOperation is
   * already in another state. See brief description of this class
   * @throws std::runtime_error will be thrown when a PixelToPixel Statement
  ...
  void reduce(std::shared_ptr<Reduction> s) {
    if (current_state != State::None && current_state != State::Reduce)
      throw std::runtime_error(
          "WindowOperation::reduce(): Window already in another state");
   ...
   * @throws std::runtime_error will be thrown when this WindowOperation is
   * already in another state. See brief description of this class
   * @throws std::runtime_error will be thrown when a LocalToPixel Statement
   * already is bound for this WindowOperation
   */
  void to_pixel(std::shared_ptr<LocalToPixel> s) {
    if (current_state != State::None && current_state != State::ToPixel)
      throw std::runtime_error(
          "WindowOperation::to_pixel(): Window already in another state");
   ...
   * @throws std::runtime_error will be thrown when this WindowOperation is not
   * in the WindowOperation::State::At or WindowOperation::State::Forall state,
   * since no output window would exist
   */
 ...
 *
 * The conditional top AST Node has to be written to If::condition and the body
 * in If::body
 */
...
/**
 * @brief Creates a new ast4vx::Add Node with \p a and \p b as the parameters
 * and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Sub Node with \p a and \p b as the parameters
 * and returns it
 */
std::shared_ptr<ast4vx::Node> operator-(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::Mul Node with \p a and \p b as the parameters
 * and returns it
 */
std::shared_ptr<ast4vx::Node> operator*(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::Div Node with \p a and \p b as the parameters
 * and returns it
 */
std::shared_ptr<ast4vx::Node> operator/(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::And Node with \p a and \p b as the parameters
 * and returns it
 */
std::shared_ptr<ast4vx::Node> operator&(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::BitwiseAnd Node with \p a and \p b as the
...
/**
 * @brief Creates a new ast4vx::Or Node with \p a and \p b as the parameters and
 * returns it
 */
std::shared_ptr<ast4vx::Node> operator|(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::BitwiseOr Node with \p a and \p b as the
 * parameters and returns it
 */
std::shared_ptr<ast4vx::Node> operator||(std::shared_ptr<ast4vx::Node> a,
                                         std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::BitwiseXor Node with \p a and \p b as the
 * parameters and returns it
 */
std::shared_ptr<ast4vx::Node> operator^(std::shared_ptr<ast4vx::Node> a,
                                        std::shared_ptr<ast4vx::Node> b);
...
/**
 * @brief Creates a new ast4vx::Not Node with \p a as the parameter and returns
 * it
 */
std::shared_ptr<ast4vx::Node> operator!(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::BitwiseNot Node with \p a as the parameter and
 * returns it
 */
std::shared_ptr<ast4vx::Node> operator~(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::ShiftLeft Node with \p a and \p b as the
 * parameters and returns it
 */
std::shared_ptr<ast4vx::Node> operator<<(std::shared_ptr<ast4vx::Node> a,
 ...
 * @brief Adds the \p statement to the statements of \p a
 */
std::shared_ptr<ast4vx::If> operator<<(std::shared_ptr<ast4vx::If> a,
                                       std::shared_ptr<ast4vx::Node> statement);
...
/**
 * @brief Creates a new ast4vx::ShiftRight Node with \p a and \p b as the
 * parameters and returns it
 */
std::shared_ptr<ast4vx::Node> operator>>(std::shared_ptr<ast4vx::Node> a,
                                         std::shared_ptr<ast4vx::Node> shift);
...
/**
 * @brief Creates a new ast4vx::Exp Node with \p a as the parameter and returns
 * it
 */
std::shared_ptr<ast4vx::Node> exp(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::Sqrt Node with \p a as the parameter and returns
 * it
 */
std::shared_ptr<ast4vx::Node> sqrt(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::Atan2 Node with \p a as the parameter and
 * returns it
 */
std::shared_ptr<ast4vx::Node> atan2(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::Abs Node with \p a as the parameter and returns
 * it
 */
...
/**
 * @brief Creates a new ast4vx::Conversion Node with \p a and \p type as the
 * parameters and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Constant Node with \p t as the type and
...
/**
 * @brief Creates a new ast4vx::VariableDefinition Node with \p a as the
 * parameter and returns it
 */
std::shared_ptr<ast4vx::Node> define(std::shared_ptr<ast4vx::Node> a);

/**
 * @brief Creates a new ast4vx::If Node with \p a as the parameter and returns
 * it
 */
...
/**
 * @brief Creates a new ast4vx::Less Node with \p a and \p b as the parameters
 * and returns it
 */
...
/**
 * @brief Creates a new ast4vx::LessEquals Node with \p a and \p b as the
 * parameters and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Equals Node with \p a and \p b as the parameters
 * and returns it
 */
...
/**
 * @brief Creates a new ast4vx::GreaterEquals Node with \p a and \p b as the
 * parameters and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Greater Node with \p a and \p b as the
 * parameters and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Unequals Node with \p a and \p b as the
 * parameters and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Vect4 Node with \p a, \p b, \p c, \p d  as the
 * parameters, \p as the type and returns it
 */
...
/**
 * @brief Creates a new ast4vx::Extract4 Node with \p a as the parameter, \p e
 * as the ast4vx::VectChannelType and returns it
 */
std::shared_ptr<ast4vx::Node> extract4(std::shared_ptr<ast4vx::Node> a,
                                       ast4vx::VectChannelType e);
 ...
 * should be the input of this ast4vx::WindowOperation
 * @param reduction_function The ast4vx::Reduction function of this
 * ast4vx::WindowOperation
 * @return The new ast4vx::WindowOperation
 */
...
/**
 * @brief Creates a new ast4vx::WindowOperation
 * @param in_bwin The WindowDesc which contains the input
 * ast4vx::WindowDescriptor
 * @param forall_function The ast4vx::Statements function of this
 * ast4vx::WindowOperation that should be applied to the whole domain
 * @return The new ast4vx::WindowOperation
...
/**
 * @brief Creates a new ast4vx::WindowOperation
 * @param in_bwins Vector of WindowDesc which contains the input
 * ast4vx::WindowDescriptor
 * @param forall_function The ast4vx::Statements function of this
 * ast4vx::WindowOperation that should be applied to the whole domain
 * @return The new ast4vx::WindowOperation
 ...
 */
template <class T>
std::shared_ptr<ast4vx::Reduction> create_reduction(
    ast4vx::Constant<T> c = ast4vx::Constant<T>()) {
...
class ASTVisitor {
 public:
  virtual ReturnType visit(std::shared_ptr<ast4vx::Node> n,
                           ParameterType p) = 0;
  /*  switch(n->type)
      {
