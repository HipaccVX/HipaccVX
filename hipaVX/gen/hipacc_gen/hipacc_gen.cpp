...
namespace generator
{
string kernel_builder(const std::vector<Kernel_Variable*>& variables, const
std::vector<string>& kernel_code, const string& kernel_name)
{
	...
	s = use_template(s, "VARIABLES_CONSTRUCTOR_INIT_LIST",
constructor_init_list); s = use_template(s, "ADD_ACCESSOR", add_accessor); s =
use_template(s, "MISC_CONSTRUCTOR", ""); s = use_template(s, "KERNEL", kernel);

    return s;
}

string kernelcall_builder(const std::vector<Kernelcall_Variable*>&
call_variables)
{
    string to_return;
...
std::tuple<std::vector<Kernelcall_Variable*>, std::vector<Kernelcall_Variable*>>
generate_iterationspace(HipaVX::Image *image)
{
...
std::tuple<std::vector<Kernelcall_Variable*>, std::vector<Kernelcall_Variable*>>
generate_accessor(HipaVX::Image *image, HipaVX::VX_Matrix *mat)
{
    std::vector<Kernelcall_Variable*> to_return_call_parameters;
    std::vector<Kernelcall_Variable*> to_return;
    ...
    case VX_TYPE_UINT8:
        for(size_t i = 0; i < mat->rows * mat->columns; i++)
			mask->flat_mask.emplace_back(std::to_string(((uint8_t*)
mat->mat.data())[i])); break; case VX_TYPE_INT32: for(size_t i = 0; i <
mat->rows * mat->columns; i++)
			mask->flat_mask.emplace_back(std::to_string(((int32_t*)
mat->mat.data())[i])); break; case VX_TYPE_FLOAT32: for(size_t i = 0; i <
mat->rows * mat->columns; i++)
			mask->flat_mask.emplace_back(std::to_string(((float_t*)
mat->mat.data())[i])); break;
    }
	...
	Kernelcall_BoundaryCondition_from_Dom *boundary = new
Kernelcall_BoundaryCondition_from_Dom(); string boundary_name = "boundary_" +
generate_image_name(image); boundary->set_real_name(boundary_name);
...
std::tuple<std::vector<Kernelcall_Variable*>, std::vector<Kernelcall_Variable*>>
generate_scalar(HipaVX::Scalar *scalar)
{
    static int id = 0;
    ...
    case VX_TYPE_INT8:
        ...
        break;
    default:
		throw std::runtime_error("std::tuple<std::vector<Kernelcall_Variable*>,
std::vector<Kernelcall_Variable*>> generate_scalar(HipaVX::Scalar
...
Kernelcall_Variable* generate_kernelcall(std::string kernel_name,
std::vector<Kernelcall_Variable*> parameters)
{
...
string node_generator(HipaVX::WriteImageNode* n, Type t)
{
    ...
    {
		string s = "\t@@@IMAGE_DATATYPE@@@ *data_@@@IMAGE@@@_@@@ID@@@ =
@@@IMAGE@@@.data();\n"; s       += "\tsave_data(@@@IMAGE_WIDTH@@@,
@@@IMAGE_HEIGHT@@@, @@@IMAGE_CHANNELS@@@, @@@CONVERSION@@@
data_@@@IMAGE@@@_@@@ID@@@, \"@@@FILENAME@@@\");\n"; s       += "\n";


        s = use_template(s, "ID", n->my_id);
        s = use_template(s, "IMAGE", generate_image_name(n->in));
		s = use_template(s, "IMAGE_DATATYPE",
VX_DF_IMAGE_to_hipacc[n->in->col]);
...
string node_generator(HipaVX::HipaccNode* n, Type t)
{
    if (t == Type::Definition)
    {
		...
		n->kernel_name = kernel.substr(kernelname_index, kernelname_end_index -
kernelname_index); const size_t kernel_name_length = n->kernel_name.length();
        while(true)
        {
            kernelname_index = kernel.find(n->kernel_name, kernelname_index);
            if (kernelname_index == string::npos)
                break;
			kernel.replace(kernelname_index, kernel_name_length, n->kernel_name
+ "_" + std::to_string(n->my_id)); kernelname_index += kernel_name_length;
        }
        return kernel;
    }
    else if (t == Type::Call)
    {
		...
		std::tuple<std::vector<Kernelcall_Variable*>,
std::vector<Kernelcall_Variable*>> tuple;

        tuple = generate_iterationspace(n->out);
		kernel_parameters.insert(kernel_parameters.end(),
std::get<0>(tuple).begin(), std::get<0>(tuple).end());
		hipacc_call.kcv.insert(hipacc_call.kcv.end(),
std::get<1>(tuple).begin(), std::get<1>(tuple).end());
        ...
        for(unsigned int i = 0; i < n->parameters.size(); i++)
        {
            ...
            case VX_TYPE_SCALAR:
                ...
                break;
            case VX_TYPE_IMAGE:
				if (i+1 < n->parameters.size() && n->parameters[i+1]->type ==
VX_TYPE_MATRIX)
                {
					tuple = generate_accessor((HipaVX::Image*) n->parameters[i],
(HipaVX::VX_Matrix*) n->parameters[i+1]); i++;
                }
                break;
            default:
				throw std::runtime_error("string
node_generator(HipaVX::HipaccNode* n, Type t):\n\tUnsupported OpenVX
Parameter");
            }
...
std::get<1>(tuple).begin(), std::get<1>(tuple).end());
		...
		hipacc_call.kcv.push_back(generate_kernelcall(n->kernel_name,
kernel_parameters)); string s = kernelcall_builder(hipacc_call.kcv);

        s = use_template(s, "ID", n->my_id);
		s = use_template(s, "BOUNDARY_CONDITION", "Boundary::UNDEFINED"); //
TODO
...
{
	//std::string hipavx_part = string("Image_") + std::to_string(i->my_id); //
same with hipacc_gen.hpp std::string hipavx_part =
::generate_image_name(i->image); return hipavx_part;
}
...
std::tuple<std::vector<Kernelcall_Variable*>, std::vector<Kernelcall_Variable*>>
generate_accessor(HipaVX::Image *image, ast4vx::Stencil *stencil)
{
    std::vector<Kernelcall_Variable*> to_return_call_parameters;
    std::vector<Kernelcall_Variable*> to_return;
...
std::tuple<std::vector<Kernelcall_Variable*>, std::vector<Kernelcall_Variable*>>
generate_accessor(Image *image)
{
		std::vector<Kernelcall_Variable*> to_return_call_parameters;
		std::vector<Kernelcall_Variable*> to_return;

		Kernelcall_BoundaryCondition_from_WH *boundary = new
Kernelcall_BoundaryCondition_from_WH(); string boundary_name = "boundary_" +
generate_image_name(image); boundary->set_real_name(boundary_name);
...
std::vector<Kernelcall_Variable*>> tuple;
	...
	kernel_parameters.insert(kernel_parameters.end(),
std::get<0>(tuple).begin(), std::get<0>(tuple).end());
	hipacc_call.kcv.insert(hipacc_call.kcv.end(), std::get<1>(tuple).begin(),
std::get<1>(tuple).end());
    ...
    for(unsigned int i = 0; i < fep->inputs.size(); i++)
    {
        ...
        /*case secret::NodeType::Variable:
			tuple = generator::generate_scalar((HipaVX::Scalar*)
n->parameters[i]); break;*//*
        case ast4vx::NodeType::Image:
        {
            auto image = std::dynamic_pointer_cast<Image>(node);
            HipaVX::Image *HVX_image = image->image;
            if (i+1 < fep->inputs.size() && fep->inputs[i+1]->type == ast4vx::NodeType::Stencil)
            {
    ...
    for (auto node: nodes)
    {
        ...
        if (subnodes.empty()) //we are at a fundamental node
        {
            ...
            case generator::Type::Call:
                sources += ast4vx::generate_call(node->kernel.get()) + '\n';
                break;
            }
        }
        else
...
std::string HipaccVisitor::visit(std::shared_ptr<ast4vx::Node> n, int i)
{
    switch(n->type)
    {
    ...
    case ast4vx::NodeType::Image:
    {
        return ast4vx::generate_image_name(std::dynamic_pointer_cast<ast4vx::Image>(n).get());
    }

    case ast4vx::NodeType::ForEveryPixel:
    {
        auto s = std::dynamic_pointer_cast<ast4vx::ForEveryPixel>(n);
        ...
        std::vector<ast4vx::Variable*> variables;

        // Dirty hack
        for(auto node: s->inputs) {
            string name="";
            if (node->type == ast4vx::NodeType::Variable)
            {
                name = visit(node, i);
            }
            if (node->type == ast4vx::NodeType::Image)
            {
                auto image = std::dynamic_pointer_cast<ast4vx::Image>(node);
                name = visit(image, i);
            }
            if (node->type == ast4vx::NodeType::Stencil)
            {
                auto s = std::dynamic_pointer_cast<ast4vx::Stencil>(node);
                name = s->name;
            }
        ...
        for(auto node_it: s->input_params)
        {
            ...
            if (node->type == ast4vx::NodeType::Variable)
            {
                type = to_string(std::dynamic_pointer_cast<ast4vx::Variable>(node)->datatype);
            }
            if (node->type == ast4vx::NodeType::Image)
            {
                auto image = std::dynamic_pointer_cast<ast4vx::Image>(node);
        ...
        if (s->output->type != ast4vx::NodeType::Image)
        ...
        def = use_template(def, "OUTPUT_DATATYPE", VX_DF_IMAGE_to_hipacc[std::dynamic_pointer_cast<ast4vx::Image>(s->output)->image->col]);
    ...
    case ast4vx::NodeType::CurrentPixelvalue:
    {
        ...
        return generate_image_name(std::dynamic_pointer_cast<ast4vx::Image>(s->subnodes[0]).get()) + "()";
    }
    ...
    {
        auto s = std::dynamic_pointer_cast<ast4vx::IterateAroundPixel>(n);
    ...
    {
        auto s = std::dynamic_pointer_cast<ast4vx::PixelvalueAtCurrentStencilPos>(n);
        std::shared_ptr<ast4vx::Stencil>stencil;
        std::shared_ptr<ast4vx::Image> image;
        ast4vx::IterateAroundPixel* iterate;
        ast4vx::ReduceAroundPixel* reduce;
        if ((iterate = dynamic_cast<ast4vx::IterateAroundPixel*>(s->parent)))
        {
            stencil = std::dynamic_pointer_cast<ast4vx::Stencil>(iterate->subnodes[1]);
            image = std::dynamic_pointer_cast<ast4vx::Image>(iterate->subnodes[0]);
        }
        else if ((reduce = dynamic_cast<ast4vx::ReduceAroundPixel*>(s->parent)))
        {
            stencil = std::dynamic_pointer_cast<ast4vx::Stencil>(reduce->subnodes[1]);
            image = std::dynamic_pointer_cast<ast4vx::Image>(reduce->subnodes[0]);
        }
    ...
    {
        auto s = std::dynamic_pointer_cast<ast4vx::StencilvalueAtCurrentStencilPos>(n);
        std::shared_ptr<ast4vx::Stencil> stencil;
        ast4vx::IterateAroundPixel* iterate;
        ast4vx::ReduceAroundPixel* reduce;
        if ((iterate = dynamic_cast<ast4vx::IterateAroundPixel*>(s->parent)))
