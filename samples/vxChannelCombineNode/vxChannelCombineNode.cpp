...
int main(int argc, char *argv[]) {
    ...
    if (context) {
        ...
        vx_image img[] = {
          ...
          vxCreateImageFromFile(context, width, height, VX_DF_IMAGE_U8, "200x300_bw_3.png"),
          vxCreateImageFromFile(context, width, height, VX_DF_IMAGE_U8, "200x300_bw_4.png"),
          vxCreateImage(context, width, height, VX_DF_IMAGE_RGBX)
        };

        if (graph) {
            vx_node nodes[] = {
                vxChannelCombineNode(graph, img[0], img[1], img[2], img[3], img[4]),
            };
        }
        ...
        if (status == VX_SUCCESS) {
            ...
            vxWriteImageAfterGraphCompletion(graph, img[4], "vxChannelCombineNode.png");
        } else {
            printf("VERIFICATION ERROR: %d\n", status);
        }

	    for (int i = 0; i < 5; i++) vxReleaseImage(&img[i]);
    }
