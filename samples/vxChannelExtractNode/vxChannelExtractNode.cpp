...
int main(int argc, char *argv[]) {
    ...
    if (context) {
        ...
        vx_image img[] = {
          vxCreateImageFromFile(context, width, height, VX_DF_IMAGE_RGBX, "200x300_rgba_1.png"),
          vxCreateImage(context, width, height, VX_DF_IMAGE_U8)
        };

        if (graph) {
            vx_node nodes[] = {
                vxChannelExtractNode(graph, img[0], VX_CHANNEL_B, img[1]),
            };
        }
        ...
        if (status == VX_SUCCESS) {
            ...
            vxWriteImageAfterGraphCompletion(graph, img[1], "vxChannelExtractNode.png");
        } else {
