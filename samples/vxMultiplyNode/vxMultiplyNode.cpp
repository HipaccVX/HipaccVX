...
int main(int argc, char *argv[]) {
    ...
    if (context) {
        ...
        if (graph) {
            vx_node nodes[] = {
                vxMultiplyNode(graph, img[0], img[1], s_scale, VX_CONVERT_POLICY_SATURATE, VX_ROUND_POLICY_TO_ZERO, img[2]),
            };
        }
        ...
        if (status == VX_SUCCESS) {
            vxWriteImageAfterGraphCompletion(graph, img[2], "vxMultiplyNode.png");
